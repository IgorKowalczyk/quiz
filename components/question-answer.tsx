     ...
     <div className="absolute inset-0 flex items-center justify-center p-12">
      <div className="grid w-full max-w-md grid-cols-2 gap-4">
       {question.options.map((option, index) => {
        ...
        return (
         <Button key={`${option.option}-${index}`} className={cn("aspect-square h-auto w-full justify-start text-white", colors[index % colors.length])} onClick={() => submitAnswer(index)}>
          <SelectedIcon fill="currentColor" className="h-1/2 w-full shrink-0" />
         </Button>
        );
       })}
      </div>
